...
const std::vector<int> kT3 = InitialiseVector(kT3Data, 20);
...
const std::vector<int> kT4 = InitialiseVector(kT4Data, 21);
...
const std::vector<int> kT5 = InitialiseVector(kT5Data, 22);
...
const std::vector<int> kT6 = InitialiseVector(kT6Data, 23);
...
int main() {
  ...
  std::vector<int> mesh_cell_array = InitialiseVector(
    kTSurface, sizeof(kTSurface) / sizeof(kTSurface[0]));
  doosabin::GeneralMesh mesh(mesh_cell_array);
  ...
  static const int kNumDelta = sizeof(kDelta) / (2 * sizeof(kDelta[0]));

  for (int i = 0; i < kNumDelta; ++i) {
    int p1;
    Eigen::Vector2d u1;
